...
const eventSchema = z
  ...
  .refine(
    (data) =>
      data.ticketType === "free" || (data.ticketPrice && data.ticketPrice > 0),
    {
      message: "Ticket price required for paid events",
      path: ["ticketPrice"],
    }
  );
        ...
        <div>
          ...
          {!hasPro && (
            <p className="text-sm text-muted-foreground mt-2">
              Free: {currentUser?.freeEventsCreated || 0}/{FREE_PLAN_LIMIT}
              events created
            </p>
          )}
        </div>
